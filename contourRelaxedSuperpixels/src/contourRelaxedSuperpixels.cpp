...
int main(int argc, char* argv[])
{
    if (argc < 2 || argc > 6)
    {
        std::cout << "Usage: " << argv[0] << " inputImage [blockWidth = 10] [blockHeight = 10] [directCliqueCost = 0.3] [compactnessWeight = 0.045 (color) / 0.015 (grayscale)]" << std::endl;
    ...
    if (image.empty()) {
        std::cout << "Input image does not exist." << std::endl;
        return 1;
	}
    ...
    int const blockWidth = (argc > 2) ? atoi(argv[2]) : 10;
    int const blockHeight = (argc > 3) ? atoi(argv[3]) : 10;
    ...
    string shortInputName = inputImage;
    int index = (int)shortInputName.find(".", 0);
    if (index == string::npos) {
      std::cout << "no . and extension in input image file" << std::endl;
      
      return 1;
    }
    shortInputName = shortInputName.substr(0, index);
    ...
    // Write grayscale image as BGR
    cv::Mat relaxedLabelBGRImage(relaxedLabelImage.size(), CV_8UC(3));
  
    for( int y = 0; y < relaxedLabelImage.rows; y++ ) {
      for( int x = 0; x < relaxedLabelImage.cols; x++ ) {
        TLabelImage tag = relaxedLabelImage.at<TLabelImage>(y, x);
        
        uint32_t R = 0;
        uint32_t G = (tag >> 8) & 0xFF;
        uint32_t B = tag & 0xFF;
        
        cv::Vec3b pixelVec(B, G, R);
        relaxedLabelBGRImage.at<cv::Vec3b>(y, x) = pixelVec;
      }
    }

    cv::imwrite(shortInputName + "_tagsImage.png", relaxedLabelBGRImage);
