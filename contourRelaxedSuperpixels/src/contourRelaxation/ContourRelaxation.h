...
class ContourRelaxation
{
    private:
        ...
        shared_ptr< GrayvalueFeature<TLabelImage> > grayvalueFeature; ///< Pointer to the grayvalue feature object, if enabled.
        bool grayvalueFeatureEnabled; ///< True if grayvalue feature is enabled.

        shared_ptr< ColorFeature<TLabelImage> > colorFeature; ///< Pointer to the color feature object, if enabled.
        bool colorFeatureEnabled; ///< True if color feature is enabled.

        shared_ptr< CompactnessFeature<TLabelImage> > compactnessFeature; ///< Pointer to the compactness feature object, if enabled.
        bool compactnessFeatureEnabled; ///< True if compactness feature is enabled.

        vector< shared_ptr< IFeature<TLabelImage> > > allFeatures; ///< Vector of pointers to all enabled feature objects.
        typedef typename std::vector< shared_ptr< IFeature<TLabelImage> > >::const_iterator FeatureIterator; ///< Shorthand for const_iterator over vector of feature pointers.
...
ContourRelaxation<TLabelImage>::ContourRelaxation(std::vector<FeatureType> features)
{
    ...
    allFeatures = std::vector< shared_ptr< IFeature<TLabelImage> > >();
        ...
        it_curFeature != features.end(); ++it_curFeature)
    {
        ...
        {
            case Grayvalue:
                grayvalueFeature = shared_ptr< GrayvalueFeature<TLabelImage> >
            ...
            case Color:
                colorFeature = shared_ptr< ColorFeature<TLabelImage> >
            ...
            case Compactness:
                compactnessFeature = shared_ptr< CompactnessFeature<TLabelImage> >
